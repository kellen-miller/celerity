...
- [x] (2026-08-01 00:23Z) Published dispositioned implementation head `47283e6`; GitHub Actions run `30675238377` passed Linux vCAN/systemd/ARM64 in 2m14s, macOS developer checks in 3m54s, and Ubuntu portable in 4m03s. The final hardware-free report now maps the reviewed behavior and explicit physical exclusions to that exact implementation evidence.
- [x] (2026-08-01 01:44Z) Published the evidence/work-item head `4abc0ea`; GitHub Actions run `30675580185` passed Linux vCAN/systemd/ARM64 in 1m45s, macOS developer checks in 2m22s, and Ubuntu portable in 3m35s. The ignored implementation briefing was generated from that clean snapshot, browser-validated at 390x844 and 1280x900 with its interactions and console clean, and accepted when the user ended Lavish session `a3b60eae4e4aa539` with “Looks good.”
...
- Observation: Responsive ownership artifacts need DOM overflow evidence in addition to visual inspection because clipped native controls and unbroken digests can preserve hidden horizontal scroll even when their boxes appear contained.
  Evidence: The first 390x844 briefing render exposed flow-select and evidence-card overflow; two local-only renderer iterations reduced both `body.scrollWidth` and `documentElement.scrollWidth` to 390, after which mobile/desktop interaction checks and console inspection passed without changing tracked source.
...
- Decision: Make `crates/control-protocol`, `crates/control-core`, and the home application deep modules; keep runtime domain areas as modules, not symmetry-driven crates.
...
- Decision: Validate canonical Run structure at HTTP admission and serialize home training admission with one SQLite immediate transaction and one-active-job uniqueness rule.
  ...
  Date/Author: 2026-08-01 / orchestrator.
- Decision: Keep the implementation ownership briefing local-only under the ignored work-item directory and require an explicit user-ended Lavish session before closing implementation.
  Rationale: The artifact transfers implementation ownership without becoming product documentation or repository state, while the explicit session end records that the human review actually completed.
  Date/Author: 2026-08-01 / orchestrator.

## Outcomes & Retrospective

The hardware-free implementation and ownership lifecycle are complete on PR #17. The result exercises the controller protocol and equivalent firmware/emulator state machines, production live single-writer runtime and fault behavior, immutable and admission-validated Run/replay lifecycle, production ONNX/`tract` model path, authority-isolated synchronization, serialized and restart-aware home training/model lifecycle, bounded exceptional notifications, startup-only two-slot activation, systemd/vCAN/ARM64 Linux integration, and the public architecture walkthrough. The sole normal closeout review reopened two missing live integrations; both were implemented before the sole cross-provider adversarial review, whose eight findings are all dispositioned. Source/evidence head `4abc0ea` passed the complete GitHub Actions matrix in run `30675580185`, and the user ended the browser-validated Lavish ownership session with “Looks good.” The ignored briefing is a local human artifact and is not part of the repository deliverable.
...
The minimal Cargo workspace has `crates/control-protocol`, `crates/control-core`, `crates/vehicle-runtime`, `crates/sync`, and `firmware/duct-controller`. `control-protocol` is `no_std` and owns exact CAN encode/decode. `control-core` owns configuration types, powertrain decoding, ordered events, canonical vehicle state/history, System Supervisor, feature reconciliation, duct policy, model inference wrapper, optimizer, controller reconciliation, Experiment execution, Run writing, and simulation/replay. Keep these as cohesive Rust modules under `crates/control-core/src/`; do not turn each noun into a crate.

The `crates/vehicle-runtime` package contains the thin `celerityd`, `celerity`, and `celerityctl` binaries plus concrete Linux adapters. `celerity simulate <scenario.toml>` and `celerity replay <run> --verify|--explore` use the production runtime. `crates/sync` produces the separate `celerity-sync` binary and has no dependency that exposes controller transport. `firmware/duct-controller` uses the shared protocol crate plus `embassy-stm32` for STM32G431 FDCAN, timers/PWM, flash, and watchdogs.
...
The contract and fixture generation commands must be explicit checked-in binaries/scripts, for example:
    ...
    cargo run -p control-protocol --features std --bin write-golden-vectors -- contracts/golden/controller-can-v1
...
Exercise the portable runtime:
    ...
    cargo run -p vehicle-runtime --bin celerity -- simulate config/scenarios/healthy-startup.toml
    cargo run -p vehicle-runtime --bin celerity -- replay fixtures/runs/healthy-startup --verify
    cargo run -p vehicle-runtime --bin celerityctl -- status --socket .tmp/celerityd.sock
...
2026-08-01: The sole cross-provider implementation adversarial review found eight verified issues. The implementation now separates event ingest and signal observation time, generates reboot-visible firmware boot sessions, treats controller configuration as intentionally volatile, fails closed on an unreadable desired baseline, validates Run v1 structure at admission, serializes and recovers home jobs durably, compares firmware/emulator behavior, byte-checks firmware golden responses, and compares bearer tokens in constant time. The corrected host and Linux vCAN matrices passed; fresh CI and the Lavish ownership loop were then completed as recorded below.

2026-08-01: Source/evidence head `4abc0ea` passed GitHub Actions run `30675580185`. A fresh implementation briefing was rendered from that clean snapshot, repaired until its 390px and 1280px browser surfaces had no horizontal overflow or console issues, exercised for ownership selection, flow navigation, retrieval feedback, source copying, and keyboard focus, and accepted through a user-ended Lavish session. Implementation and its ownership handoff are complete; physical/HIL, installed-bus, mechanism, thermal/model-envelope, and road evidence remain explicitly external and unclaimed.
