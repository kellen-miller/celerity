...
use celerity::DiagnosticsSnapshot;
use celerity_protocol::{Frame, decode, encode};
...
fn live_kernel_sockets_drive_model_command_and_seal_run() {
    ...
    let mut runtime = celerity::LiveRuntime::open_virtual_hardware_free(
        bundle,
        Some(model),
...
fn send_controller_wakeup() {
    ...
    let encoded = encode(
        &Frame::DiscoveryProbe(celerity_protocol::DiscoveryProbe {
            protocol_major: 1,
            protocol_minor: 0,
            flags: 0,
            probe_sequence: 0,
